...
const signup = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, token, user: { id: user._id, name: user.name, email: user.email, role: user.role, points: user.points } });
  } catch (error) {
    console.error(error);
...
const login = async (req, res) => {
  try {
    ...
    const token = generateToken(user);
    res.json({ success: true, token, user: { id: user._id, name: user.name, email: user.email, role: user.role, points: user.points } });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
const forgotPassword = async (req, res) => {
  try {
    ...
    // Create reset url
    // Frontend URL hardcoded here for simplicity, change port if needed
